...
description: Diagnose the liquidity bot's health (hedging, rebalancing, errors, overall status). Default mode downloads a snapshot via `staging-status`/`prod-status`; add `remote` to skip the download and query the live server directly.
argument-hint: <prod|staging> [remote]
---

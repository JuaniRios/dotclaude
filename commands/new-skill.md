---
allowed-tools: Bash(gt:*), Bash(git:*), Bash(test:*), Bash(ls:*), Read, Write, Edit, Glob, Grep, AskUserQuestion
description: Create a new Claude Code skill or slash command in the dotclaude repo (git-tracked, symlinked to ~/.claude). Handles file creation, commits via gt, and offers to push. Use /new-skill to add a new skill or command.
...
```bash
git add ~/Github/dotclaude/commands/<name>.md
# or
git add ~/Github/dotclaude/skills/<name>/SKILL.md
```

Then commit:

```bash
gt create add-<type>-<name> -m "feat: add /<name> <type>"
```

If already on a non-trunk branch, use `gt modify --commit` instead of
`gt create`.
...
Want me to push with `gt submit`?
```

Wait for confirmation. If yes, run `gt submit`. If no, stop.
...
3. Use `gt` for all git mutations, never raw `git commit` or `git push`.
